...
export default defineEventHandler(async (event) => {
  ...
  const url = `${m2mApiUrl}/devices/${deviceId}/logs`
  console.log('[M2M Proxy] GET', url)

  try {
    const response = await $fetch(url)
    console.log('[M2M Proxy] Success device logs', { deviceId })
    return response
  } catch (error: any) {
    console.error('[M2M Proxy] Failed device logs', { deviceId, details: buildErrorDetails(error) })
    throw createError({
      statusCode: error?.statusCode || error?.response?.status || 500,
      statusMessage: error?.statusMessage || error?.response?.statusMessage || 'Failed to fetch device logs'
    })
  }
})
